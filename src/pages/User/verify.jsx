...
function Verify(){
    ...
    async function handleFormSubmit(event) {
        ...
        const res = await fetch('http://localhost:8080/api/verify', {
            ...
            credentials: 'include',
            body: await JSON.stringify({email : location.state.email , verificationCode : formState.verificationCode}),
        });
            ...
            <div className="form-container">
                ...
                <form>
                    <div className="input-div">
                        <input type = "number" id ="verificationCode" className="form-input" placeholder = "Verification Code" value = {formState.verificationCode} onChange={handleFormState} required></input>
                        {isEmpty.verificationCode && <p className="error">Verification Code is required</p>}
                    </div>
                    {!(error === "") && <p className="error">! {error}</p>}
                    <button onClick={handleFormSubmit} className="form-button">VERIFY</button>
                    <br></br>
                </form>
            </div>
        </div>
